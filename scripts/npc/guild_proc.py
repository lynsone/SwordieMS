...
if chr.isGuildMaster():
    if guild.getMaxMembers() < GameConstants.MAX_GUILD_MEMBERS:
        response = sm.sendAskYesNo("Would you like to increase your guild's max capacity? You currently have "
                        + str(guild.getMaxMembers()) + " members, but I can increase it by 10 for 500 thousand mesos.")

elif guild is None:
    response = sm.sendAskYesNo("Would you like to create a guild? This will cost 5 million mesos.")
else:
